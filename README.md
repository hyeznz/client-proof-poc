# 이사짐캐리 후기 페이지

이사짐캐리를 실제로 이용한 고객님들의 후기를 한눈에 모아 보는 페이지입니다.

<p align="center">
  <img src="docs/screens/hero.png" alt="이사짐캐리 후기 페이지 모바일 화면" width="320">
</p>

## 바로 보기

**https://client-proof-poc.vercel.app**

## 이 페이지는

이사를 앞두고 업체를 고르는 분들을 위한 페이지입니다. 이사짐캐리를 이용한 고객님들이 직접 남긴 후기를 카테고리별로 모아 보여주고, 마음에 드는 후기를 눌러 전체 내용과 사진을 볼 수 있습니다. 여기서 바로 방문 견적도 신청할 수 있습니다.

## 주요 내용

- 실제 이용 고객이 직접 남긴 후기
- 후기 사진 첨부 지원
- "하이라이트", "가격", "재이사맛집" 등 상황별 카테고리로 후기 탐색
- 후기를 보다가 바로 방문 견적 신청
- 회원가입·앱 설치 없이 모바일 브라우저에서 바로 이용
- 이용 고객이라면 직접 후기 작성 가능

## 이용 흐름

```mermaid
flowchart LR
    A[페이지 접속] --> B[후기 그리드 탐색]
    B --> C[후기 상세 보기]
    C --> D[방문 견적 신청]
    D --> E[접수 완료]
```

| 단계 | 보이는 것 | 할 수 있는 것 |
|---|---|---|
| 후기 그리드 | 카테고리별 후기 카드 | 원하는 카테고리 탭 선택, 카드 넘겨보기 |
| 후기 상세 | 후기 전문, 첨부 사진 | 실제 이용 후기 자세히 읽기 |
| 방문 견적 신청 | 이사 정보 입력 폼 | 이름·연락처·이사 정보 입력 후 신청 |
| 접수 완료 | 접수 안내 | 담당자 연락 대기 |

<p align="center">
  <img src="docs/screens/01-grid.png" alt="후기 그리드" width="200">
  <img src="docs/screens/02-detail.png" alt="후기 상세" width="200">
  <img src="docs/screens/03-quote.png" alt="견적 신청 폼" width="200">
</p>

주로 인스타그램 프로필 링크, 카카오톡 상담 답장, 명함 QR코드를 통해 접속합니다. 견적 신청은 1~2분이면 끝나며, 접수 후 담당자가 순차적으로 연락드립니다.

## 자주 묻는 질문

**서비스 지역이 어디인가요?**
출발지(짐이 나가는 곳) 기준 서울 및 수도권입니다.

**후기는 누구나 쓸 수 있나요?**
이사짐캐리를 실제로 이용한 고객님만 작성할 수 있습니다. 남겨주신 후기는 운영자 확인 후 페이지에 게재됩니다.

**견적 신청하면 언제 연락이 오나요?**
보통 1일 이내에 연락드립니다. 주말·휴일이 끼거나 현장 일정이 몰린 경우 이틀 이상 걸릴 수 있으며, 늦어도 3일 안에는 연락드리고자 합니다.

**연락처가 어떻게 되나요?**
연락처와 사업자 정보는 추후 업데이트 예정입니다.

## Tech

Next.js · Supabase

기존에는 HTML 파일 한 장으로 운영하던 페이지였습니다. 2026년 8월, 고객이 직접 후기를 남기고 운영자가 검토 후 게재할 수 있도록 Next.js와 Supabase 기반으로 새로 만들었습니다.

## License / Contact

이사짐캐리 소유이며 무단 사용을 금합니다. 문의처는 추후 업데이트 예정입니다.

## 앞으로 다듬을 것

- 후기를 날짜순으로도 볼 수 있게 하여, 이용자가 최근 흐름을 더 구체적으로 파악할 수 있도록
- 상단 안내 문구(콜아웃)의 모션 효과와 내용 보완
- 인스타그램에서 홈페이지로 자연스럽게 이어지도록 연동 정리
- 이 페이지를 별도 운영할지, 홈페이지 안으로 흡수할지 검토 중
- 후기·견적 접수 경로의 보안과 안정성 보강
